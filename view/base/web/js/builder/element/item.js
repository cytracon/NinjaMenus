define([
	...
	var directive = function(cytraconBuilderUrl, cytraconBuilderService, $compile) {
		return {
      		replace: true,
			templateUrl: function(elem) {
				return cytraconBuilderUrl.getViewFileUrl('Cytracon_NinjaMenus/js/templates/builder/element/item.html');
			},
			controller: function($rootScope, $scope, $controller) {
				...
				var submenuHtml = '<div ng-class="mgz.getInnerClasses()" ng-if="submenuLoaded&&element.elements.length" ng-include="::$root.cytraconBuilderUrl.getViewFileUrl(\'Cytracon_Builder/js/templates/builder/element/list.html\')"dnd-disable-builder="!element.builder.is_collection"dnd-disable-if="element.builder.dndDisabled" dnd-list="element.elements"dnd-drop="mgz.dropElement(item, index, element)"dnd-allowed-types="::element.builder.allowed_types"></div>';
